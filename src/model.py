...
# https://github.com/avBuffer/UNet3plus_pth/tree/master/unet
...
class UNet(nn.Module):
    def __init__(self, num_classes=29):
        ...
        # @@ 논문에서는 padding을 0으로 두었지만 이 코드 구현에서는 1로 두고 있다
        ...
        self.enc5_2 = CBR2d(
            in_channels=1024, out_channels=1024, kernel_size=3, stride=1, padding=1, bias=True
        )
        self.unpool4 = nn.ConvTranspose2d(
            in_channels=1024, out_channels=512, kernel_size=2, stride=2, padding=0, bias=True
        )
        # (input_size - 1) * stride + kernel_size - 2 * padding
        # => (input_size - 1) * 2 + 2 = 2 * input_size => 2배 upsampling
        ...
        self.unpool3 = nn.ConvTranspose2d(
            in_channels=512, out_channels=256, kernel_size=2, stride=2, padding=0, bias=True
        )
...
class EffiUNet3Plus(nn.Module):
    def __init__(
        self, n_channels=3, n_classes=29, bilinear=True, feature_scale=4, is_deconv=True, is_batchnorm=True
    ):
        super().__init__()
        ...
        # output
        self.outup = nn.Upsample(scale_factor=2, mode="bilinear")
    ...
    def forward(self, inputs):
        ...
        hd4 = self.relu4d_1(
            self.bn4d_1(
                self.conv4d_1(torch.cat((h1_PT_hd4, h2_PT_hd4, h3_PT_hd4, h4_Cat_hd4, hd5_UT_hd4), 1))
            )
        ...
        hd3 = self.relu3d_1(
            self.bn3d_1(
                self.conv3d_1(torch.cat((h1_PT_hd3, h2_PT_hd3, h3_Cat_hd3, hd4_UT_hd3, hd5_UT_hd3), 1))
            )
        ...
        hd2 = self.relu2d_1(
            self.bn2d_1(
                self.conv2d_1(torch.cat((h1_PT_hd2, h2_Cat_hd2, hd3_UT_hd2, hd4_UT_hd2, hd5_UT_hd2), 1))
            )
        ...
        hd1 = self.relu1d_1(
            self.bn1d_1(
                self.conv1d_1(torch.cat((h1_Cat_hd1, hd2_UT_hd1, hd3_UT_hd1, hd4_UT_hd1, hd5_UT_hd1), 1))
            )
        )  # hd1->320*320*UpChannels

        d1 = self.outconv1(self.outup(hd1))  # d1->320*320*n_classes
        # return F.sigmoid(d1)
